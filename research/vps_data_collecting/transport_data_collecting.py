...
for index, row in df.iterrows():
    ...
    transport_context = client.get_transportation_context(
        long=long,
        lat=lat
    )
    if transport_context.success:
        extended_df.at[index, "metro_count"] = transport_context.metro_stations_count
        if transport_context.nearest_metro_distance is not None:
            extended_df.at[index, "metro_distance"] = transport_context.nearest_metro_distance
        extended_df.at[index, "bus_tram_count"] = transport_context.total_public_transport
        if transport_context.nearest_bus_distance is not None:
            extended_df.at[index, "bus_distance"] = transport_context.nearest_bus_distance
    current_index += 1
