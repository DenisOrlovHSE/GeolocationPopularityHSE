...
for index, row in df.iterrows():
    ...
    ad_context = client.get_administrative_context(
        long=long,
        lat=lat
    )
    if ad_context.success:
        extended_df.at[index, "is_urban"] = 1 if ad_context.is_urban else 0
        if ad_context.city is not None:
            extended_df.at[index, "city"] = ad_context.city
        if ad_context.region is not None:
            extended_df.at[index, "region"] = ad_context.region
    current_index += 1
