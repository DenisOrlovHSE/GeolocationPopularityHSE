...
def initialize_extended_df(initialized: bool) -> pd.DataFrame:
    if os.path.exists(EXTENDED_DATASET_PATH) and initialized:
        ...
        df["parking_count"] = 0
        df["parking_types"] = "empty"
        df["parking_distance"] = 0.0
        return df
...
for index, row in df.iterrows():
    ...
    parking_context = client.get_parking_context(
        long=long,
        lat=lat
    )
    if parking_context.success and parking_context.nearest_parking_distance is not None:
        extended_df.at[index, "parking_count"] = parking_context.parking_count
        extended_df.at[index, "parking_types"] = json.dumps(parking_context.parking_types)
        extended_df.at[index, "parking_distance"] = parking_context.nearest_parking_distance
    current_index += 1
